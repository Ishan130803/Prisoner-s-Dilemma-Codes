...
class EvaluationEngine:
    ...
    def __init__(self, player1, player2):
        ...
        self.error = 0.05 + random.random() * 0.05
    ...
    def event_loop(self):
        while self.iteration <= self.rounds:
            ...
            t1 = threading.Thread(
                target=self.threaded_player_call, args=(0, self.streak, self.iteration)
            )
            t2 = threading.Thread(
                target=self.threaded_player_call, args=(1, self.streak, self.iteration)
            )
            t1.start()
            t2.start()
            ...
            if not self.move_queue[0] or self.move_queue[0] not in [
                EvaluationEngine.COOP,
                EvaluationEngine.DEFECT,
            ]:
                if self.iteration == 1:
                    self.move_queue[0] = random.choice(
                        [EvaluationEngine.COOP, EvaluationEngine.DEFECT]
                    )
                else:
                    self.move_queue[0] = self.history[self.iteration - 1][1]

            if not self.move_queue[1] or self.move_queue[1] not in [
                EvaluationEngine.COOP,
                EvaluationEngine.DEFECT,
            ]:
                if self.iteration == 1:
                    self.move_queue[1] = random.choice(
                        [EvaluationEngine.COOP, EvaluationEngine.DEFECT]
                    )
                else:
                    self.move_queue[1] = self.history[self.iteration - 1][2]
            current_error = self.error
            current_error = self.error - self.streak // 4 * 0.01 if self.error - self.streak//4 >= 0.01 else 0.01
            # errors in communication with agents
            is_err1 = random.random() < current_error
            is_err2 = random.random() < current_error
          ...
          """
            ...
            if (
                self.move_queue[0] == EvaluationEngine.COOP
                and self.move_queue[1] == EvaluationEngine.COOP
            ):
                self.score[1] += self.payoff_matrx[0][0](self.streak)
                self.score[2] += self.payoff_matrx[0][1](self.streak)
                self.streak += 1
            elif (
                self.move_queue[0] == EvaluationEngine.DEFECT
                and self.move_queue[1] == EvaluationEngine.COOP
            ):
                ...
                self.move_queue[0] == EvaluationEngine.COOP
                and self.move_queue[1] == EvaluationEngine.DEFECT
            ):
