...
class AngryMan_v4(BaseAgent):
    def __init__(
        ...
        self.name = f'{name}_bf_{base_forgiveness}__be_{base_exponent}__bs_{base_streak}'
...
class Tester(BaseAgent):
    ...
    def next_move(self,state):
        ...
        if (mylastMove == 1 and oplastMove == 1):
            ...
            self.counted_round += 1
            self.cooperation_ratio = 1 if self.cooperation_count / self.counted_round > 1 else self.cooperation_count / self.counted_round
    ...
    def next_move(self,state):
        ...
        if (itr > 2 and history[itr-2][self.id] == 1 and oplastMove == 1):
            ...
            if (self.honest_probability % 5 == 0):
                self.dishonest_probability -= 1 if self.dishonest_probability > -1 else 0
        ...
        if (itr > 2 and oplastMove == -1 and mylastMove == -1):
            ...
            if self.honest_probability > 40 and self.deadlock >= 2 and not self.recently_forgave:
...
class TitForTatWdld(BaseAgent):
    ...
    def next_move(self,state):
        ...
        mylastMove = state['history'][itr-1][self.id]
        oplastMove = state['history'][itr-1][op_id]
        print(self.honest_probability, self.dishonest_probability, self.total_dishonesty)
        ...
        if self.total_dishonesty >= 4:
            self.forgiveness = 0
        if itr > 4 and history[itr-3][self.id] == 1 and history[itr-2][op_id] == 1 and history[itr-2][self.id] == -1 and history[itr-1][op_id] == -1:
